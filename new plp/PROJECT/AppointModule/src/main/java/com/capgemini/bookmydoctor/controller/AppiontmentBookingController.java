...
public class AppiontmentBookingController {
	...
	@PostMapping(path = "/add-appiontment/{appointmentId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public AppiontmentBookingResponse addAppiontment(@RequestBody AppiontmentBooking appiontment, @PathVariable("id") int appointmentId) {
		System.out.println(appiontment);
		AppiontmentBookingResponse response = new AppiontmentBookingResponse();
		if (service.addAppiontment(appiontment,appointmentId)) {

...
public class AppiontmentBookingController {
	...
	@PostMapping(path = "/template/add-appiontment/{appointmentId}", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public AppiontmentBookingResponse addAppiontment(@RequestBody AppiontmentBooking appiontment,@PathVariable("appointmentId") int appointmentId) {
		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<AppiontmentBooking> entity = new HttpEntity<AppiontmentBooking>(appiontment, headers);

		return restTemplate.exchange("http://localhost:8081/add-appiontment"+appointmentId, HttpMethod.POST, entity,
